...
var diners:number=Number(prompt("Quants diners tens?"));

//Preguntem si volen introduïr un producte
var pregunta:string=prompt("Vols introduir un producte?") as string;
...
do{
	...
	const nom_producte:string=prompt("Quin és el nom del producte?") as string;
	const preu_producte:number=Number(prompt("Quin és el preu del producte?"));
	let quantitat_producte:number=Number(prompt("Quina és la quantitat del producte?"));

	//Confirmem que es tot correcte
	var correcte:string=prompt("És tot correcte?") as string;
	...
	if (correcte == "Si"){
    	...
    	let preu_total:number= preu_producte * quantitat_producte;
