"use strict";
function exercici1() {
    let input1 = document.getElementById("camp1").value; //registramos el valor del primer input en una variable de forma explicita(lo registro como string)
    let input2 = document.getElementById("camp2").value; //registramos el valor del segundo input en una variable de forma explicita(lo registro como string)
    //los convertimos a tipos number sus valores
    let input1GG = parseInt(input1);
    let input2GG = parseInt(input2);
...
function exercici2() {
    ...
    for (let i = 0; i < input1GG; i++) { //bucle que genera numeros aleatrios que termina cuando llega al valor asignado en el primer input
        let number = Math.floor(Math.random() * (75 - 1) + 1); //aqui tenemos una variable donde su valor es un numero aleatorio entre 1 y 75
